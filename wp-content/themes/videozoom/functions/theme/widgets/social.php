...
function widget( $args, $instance ) {
		...
		<ul class="social">
			...
			foreach ($socialProfiles as $item)
			{
				...
				if ($instance[$id] && $instance[$id] != '') {
				    ...
				    echo '<li><a class="'.$id.'" href="'.$instance[$id].'" rel="external,nofollow" title="'.$instance[$iditem].'"><img src="'. get_template_directory_uri() .'/images/icons/social_widget/'.strtolower($item).'.png" alt="'.$instance[$iddesc]. '" />'.$instance[$iditem].'<span>'.$instance[$iddesc].'</a></li>'; 
				}
			} 
			?>
  		</ul>
    ...
    ?>
		...
		foreach ($socialProfiles as $item)
		{
			...
			?>
 	 		<p>
				<img style="float: left; margin:0 8px 0 0;" src="<?php echo get_template_directory_uri(); ?>/images/icons/social_widget/<?php echo $id; ?>.png" />
